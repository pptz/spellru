...
const RussianWordGame = () => {
  ...
  // Initialize the game only if no saved state exists
  useEffect(() => {
    if (dictionaryLoaded && gameLetters.length === 0) {
      startNewGame();
    }
  }, [dictionaryLoaded]);
  ...
  // Check if a word can be formed with the available letters (with duplicates allowed)
  ...
  const canFormValidWords = (letters) => {
    ...
    validWords.forEach(word => {
      ...
      [...word].forEach(char => {
        ...
        for (let i = 0; i < letters.length; i++) {
          if (letters[i] === char && !usedIndices.has(i)) {
            letterUsage.set(`${char}-${i}`, true);
            usedIndices.add(i);
            break;
          }
        }
    ...
    // Check if all letter positions are used
  ...
  const generateViableLetterSet = () => {
    ...
    while (attempts < maxAttempts) {
      ...
      // Select 7 random letters (duplicates allowed)
      const selectedLetters = [];
      for (let i = 0; i < 7; i++) {
        const randomIndex = Math.floor(Math.random() * allLetters.length);
        selectedLetters.push(allLetters[randomIndex]);
      }
  ...
  const handleSubmit = () => {
    ...
    if (!canFormWord(currentWord, gameLetters)) {
      setMessage('Вы не можете использовать буквы больше, чем они доступны');
      setCurrentWord('');
      return;
    }
